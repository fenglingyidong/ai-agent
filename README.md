...
会话级短期导购偏好保存到 Redis，Key 前缀为 `shopping:preference:`，TTL 由 `app.shopping.preference-ttl` 控制。偏好状态覆盖品类、预算、品牌、尺码、颜色、风格和使用场景。

每轮 `/api/react` 请求都会先加载当前偏好，并把压缩后的偏好摘要提供给小模型路由、简单任务快车道和主 Agent。系统会从用户本轮输入、路由槽位和视觉上下文中抽取明确偏好并合并回 Redis；模型也可以通过 `updateShoppingPreference` 工具补充偏好。

当用户显式修改偏好时，以本轮表达为准；如果品类切换，会清理品牌、尺码、颜色、风格、使用场景等强相关旧字段，避免旧需求继续影响后续导购。
