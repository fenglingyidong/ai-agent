...
- **短期偏好状态**：`ShoppingStateService` 持有 `ShoppingPreferenceState`（品类、预算、品牌、尺码、颜色、风格、使用场景），由 `BuiltInTools.updateShoppingPreference` 工具维护，并被 `SimpleTaskAgent` 在 system prompt 中复用。
