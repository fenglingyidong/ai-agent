...
Agent 效果评测
现在只有 DuReader Recall 评测，项目文档也承认不能证明电商导购效果。需要补 ESCI/SQID 或自建电商样例集，指标至少包括 Recall@K、NDCG@10、MRR、工具调用准确率、槽位补全率、加购成功率、幻觉率、平均 Token、延迟。
...
- 已完成：主 Agent 通过 `toolContext(...)` 传递 `userId/sessionId`；`BuiltInTools.updateShoppingPreference` 直接读取 Spring AI `ToolContext`；已删除自研 `ToolExecutionContext`。
- 已完成：`MallMcpClient` 改为 `McpSyncClient + WebClientStreamableHttpTransport`，不再手写 MCP initialize、JSON-RPC 请求和 SSE/text 解析。
- 已完成：`MallMcpToolCallback` 改为基于 Spring AI `SyncMcpToolCallbackProvider` 发现并注册 `mall_*` 工具；上下文注册前移到路由阶段，主 Agent 不再改写 MCP 工具参数和返回值。
...
- 已删除：`MallTokenFilter` / `MallTokenContext`，商城 token 只通过 `ChatController -> ShoppingRouteExecutor -> MallMcpContextClient` 显式注册给 `mall-mcp`。
...
- 已完成：主 Agent 的商城 MCP 工具注册改为 `SyncMcpToolCallbackProvider`，不再由 RAGAgent 手写 `mall_*` 工具 schema 和 `FunctionToolCallback`。
- 已精简：`MallMcpToolCallback` 只负责初始化 MCP client、发现并过滤 `mall_*` 工具；不再包裹每次工具调用、不再向 arguments 注入 `sessionId`、不再还原 Spring AI MCP 返回值。
- 已补齐：本轮需要商城工具但工具发现失败或未发现 `mall_*` 工具时，主 Agent 直接返回“商城 MCP 调用失败”，避免无工具时继续生成。
- 暂保留：`MallMcpOperations` 仍服务 `SimpleTaskAgent` 快车道的确定性工具调用，避免把简单任务链路一起改重。
- 已验证：`MallMcpToolCallbackTest,ReActAgentTest,SimpleTaskAgentTest` 通过。

### 任务策略模板 / 类 skill 机制

- 已完成：代码中新增轻量 `ShoppingTaskPolicy` / `ShoppingTaskPolicyRegistry`，不引入重量级 Skill 框架，也不拆分多个独立 Agent。
- 已完成：Planner 输出 `task_policies / missing_slots / tool_candidates / need_confirm / risk_level`；Registry 归一化为可组合任务策略；路由层把策略传给主 Agent。
- 已完成：`ReActAgent` 在系统提示词中组合策略 prompt 片段，并按策略过滤商品知识库、偏好更新和 `mall_*` MCP 等导购受控工具。
- 简历口径：建议写“通过 Planner 选择可组合导购任务策略模板”，避免直接声称已有独立 skills 框架。
